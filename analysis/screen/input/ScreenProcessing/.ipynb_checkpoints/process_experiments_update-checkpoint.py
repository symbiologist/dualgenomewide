...
def processExperimentsFromConfig(configFile, libraryDirectory, generatePlots='off'):
    ...
    if len(exptParameters['analyses']) > 0:
        ...
        geneTable = pd.concat(analysisTables, axis=1, sort=True).reorder_levels([1,2,0],axis=1).sort_index(axis=1) # dwu add sort arg due to pandas update
...
def scoreGeneByBestTranscript(geneTable):
    ...
    return pd.concat(bestTransList, axis=1, keys=tupList, sort=True) # dwu add sort arg due to pandas update
